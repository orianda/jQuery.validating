(function ($) {
    ...
    function remove(selector, validator) {
        var amount = 0;
        if (selector && $.isFunction(validator)) {
            $.each(registry, function (index, entry) {
                if (entry.selector === selector && (entry.validator === validator || !entry.fix && typeof validator === 'undefined')) {
    ...
    function isPromise(promise) {
        return promise instanceof Object && typeof promise.then === 'function' && promise.then.length >= 2;
    }
